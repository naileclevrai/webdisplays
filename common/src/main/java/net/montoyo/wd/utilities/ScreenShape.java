...
public final class ScreenShape {
    ...
    public static final class Data {
        ...
        public final ScreenPieceType[] pieces;
        ...
        public ScreenPieceType getPiece(int x, int y) {
    ...
    public static Data compute(BlockGetter level, BlockPos origin, BlockSide side, Vector2i size, ScreenShapeMode mode, BlockPos seed) {
        ...
        ScreenPieceType[] pieces = new ScreenPieceType[width * height];
        for (int i = 0; i < pieces.length; i++)
            pieces[i] = ScreenPieceType.FULL;
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                pos.toBlock(bp);
                BlockState cellState = level.getBlockState(bp);
                boolean isScreen = ScreenBlocks.isScreen(cellState);
                raw[y * width + x] = isScreen;
                if (isScreen)
                    pieces[y * width + x] = ScreenBlocks.getPiece(cellState);
                pos.add(side.right);
            }
        ...
        return new Data(mask, width, height, heights, bottoms, rowLefts, rowRights, boundary, topLeft, topRight, bottomLeft, bottomRight, leftBottom, leftTop, rightBottom, rightTop, smoothMask, pieces);
    }
    ...
    public static Vector3i findConnectedScreenEntity(BlockGetter level, BlockPos start, BlockSide side, int maxWidth, int maxHeight) {
        ...
        while (head < tail) {
            ...
            if (ScreenBlocks.isScreen(level.getBlockState(bp))
    ...
    public static boolean isInside(Data data, ScreenShapeMode mode, float localX, float localY) {
        ...
        if (data.hasBlock(x, y)) {
            float fx = localX - x;
            float fy = localY - y;
            return data.getPiece(x, y).contains(fx, fy);
        }
    ...
    private static boolean isScreenAt(BlockGetter level, int ox, int oy, int oz, BlockSide side, int x, int y, BlockPos.MutableBlockPos bp) {
        ...
        return ScreenBlocks.isScreen(level.getBlockState(bp));
    }
