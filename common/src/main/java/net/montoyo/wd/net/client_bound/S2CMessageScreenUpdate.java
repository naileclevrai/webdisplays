...
public class S2CMessageScreenUpdate extends Packet  {
    ...
    public static S2CMessageScreenUpdate shapeMode(ScreenBlockEntity screen, BlockSide side, ScreenShapeMode mode) {
        S2CMessageScreenUpdate screenUpdate = new S2CMessageScreenUpdate(screen.getBlockPos(), side);
        screenUpdate.control = new ScreenModifyControl(mode);
        return screenUpdate;
    }

    public static S2CMessageScreenUpdate size(ScreenBlockEntity screen, BlockSide side, Vector2i size) {
        S2CMessageScreenUpdate screenUpdate = new S2CMessageScreenUpdate(screen.getBlockPos(), side);
        screenUpdate.control = new ScreenModifyControl(ScreenModifyControl.ControlType.SIZE, size);
        return screenUpdate;
    }
    ...
    public static S2CMessageScreenUpdate link(ScreenBlockEntity screen, BlockSide side, String linkId, ScreenLinkMode mode, boolean origin) {
        S2CMessageScreenUpdate screenUpdate = new S2CMessageScreenUpdate(screen.getBlockPos(), side);
        screenUpdate.control = new ScreenLinkControl(linkId, mode, origin);
        return screenUpdate;
    }

    public static S2CMessageScreenUpdate refresh(ScreenBlockEntity screen, BlockSide side) {
        S2CMessageScreenUpdate screenUpdate = new S2CMessageScreenUpdate(screen.getBlockPos(), side);
        screenUpdate.control = new ScreenRefreshControl();
        return screenUpdate;
    }
    ...
    public void handle(NetworkEvent.Context ctx) {
        if (checkClient(ctx)) {
            ctx.enqueueWork(() -> {
                ...
                } else if (control instanceof TurnOffControl) {
                    WebDisplays.PROXY.forceDisableScreen(pos, side);
                }
            });
